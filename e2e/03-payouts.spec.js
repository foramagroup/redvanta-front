...
test.describe('Payout flow (affilié + admin + webhook)', () => {
  test('create payout request as affiliate and approve as admin, then simulate webhook', async ({ page, baseURL }) => {
    ...
    const createRes = await axios.post(`${process.env.E2E_API_BASE || 'http://localhost:4000'}/admin/affiliates`, { name: "PW Test Aff", email: "pw@aff.test" }, { headers: { Authorization: `Bearer ${adminToken}` } });
    const aff = createRes.data.affiliate;
    expect(aff).toBeDefined();

    // 2) create payout request via affiliate (or admin)
    const payoutRes = await axios.post(`${process.env.E2E_API_BASE || 'http://localhost:4000'}/admin/payouts/request`, { affiliateId: aff.id, amountCents: 1500, currency: "EUR", note: "Test payout" }, { headers: { Authorization: `Bearer ${adminToken}` }});
    const payout = payoutRes.data.payout;
    expect(payout).toBeDefined();

    // 3) Approve and process payout (admin)
    const approveRes = await axios.post(`${process.env.E2E_API_BASE || 'http://localhost:4000'}/admin/payouts/${payout.id}/approve`, {}, { headers: { Authorization: `Bearer ${adminToken}` }});
    ...
    const detail = await axios.get(`${process.env.E2E_API_BASE || 'http://localhost:4000'}/admin/payouts/${payout.id}`, { headers: { Authorization: `Bearer ${adminToken}` }});
