...
import { 
  Building2, Link2, Shield, CreditCard, Upload, Eye, EyeOff, 
  Copy, ExternalLink, Plus, Globe, CheckCircle, Smartphone, 
  Mail, Key, ZoomIn, ZoomOut, Info, Loader2 
} from "lucide-react";
...
const Settings = () => {
  ...
  // Hook useSettings pour subscription
  const {
    subscription,
    invoices,
    availablePlans,
  ...
  const handleCopy = (text) => {
    ...
    toast({ 
      title: t("sett.copied"), 
      description: t("sett.copied_desc") 
    });
  };
  ...
  // Charger plans disponibles quand modal upgrade s'ouvre
  useEffect(() => {
    if (showUpgrade && availablePlans.length === 0) {
      fetchAvailablePlans();
    }
  }, [showUpgrade, availablePlans.length, fetchAvailablePlans]);
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      if (uploadedLogo) {
        setLogoPreview(
          uploadedLogo.startsWith("http") 
            ? uploadedLogo 
            : `${apiBase}${uploadedLogo}`
        );
      }
  ...
  // Sauvegarder platforms settings
    ...
    <DashboardLayout title={t("sett.title")} subtitle={t("sett.subtitle")}>
        ...
        <Tabs defaultValue="general" className="w-full">
          <TabsList className="w-full justify-start mb-6 bg-secondary/50 p-1">
            ...
            <TabsTrigger value="subscription" className="gap-2">
              <CreditCard size={14} /> {t("sett.subscription")}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="general" className="space-y-6">
            <motion.div variants={fadeUp} custom={0} className="rounded-xl border border-border/50 bg-gradient-card p-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        ...
                        {logoPreview && (
                          <div className="flex items-center gap-2">
                            <button 
                              onClick={() => setLogoScale(Math.max(50, logoScale - 10))} 
                  ...
                  <div>
                    ...
                    <Input 
                      ...
                      onChange={(e) => updateGeneralField("fullName", e.target.value)} 
                    />
                  </div>

                  <div>
                    <label className="text-xs text-muted-foreground mb-1.5 block">
                      {t("sett.business_name")}
                    </label>
                    <Input 
                      className="bg-secondary/50 border-border/50" 
                  ...
                  <div>
                    ...
                    <Input 
                      ...
                      value={generalForm.vatNumber} 
                      onChange={(e) => updateGeneralField("vatNumber", e.target.value)} 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input 
                      ...
                      value={generalForm.tradeNumber} 
                      onChange={(e) => updateGeneralField("tradeNumber", e.target.value)} 
                    />
                  </div>

                  <div>
                    <label className="text-xs text-muted-foreground mb-1.5 block">
                      {t("sett.contact_email")}
                    </label>
                    <Input 
                      className="bg-secondary/50 border-border/50" 
              ...
              >
                {isSavingGeneral && <Loader2 size={14} className="mr-2 animate-spin" />}
                {t("sett.save_changes")}
              </Button>
            </motion.div>
          </TabsContent>
          ...
          <TabsContent value="platforms">
            <motion.div variants={fadeUp} custom={0} className="rounded-xl border border-border/50 bg-gradient-card p-6">
                ...
                {[
                  ...
                  <div key={platform.labelKey}>
                    ...
                    <div className="flex gap-2">
                      <Input 
                        className="bg-secondary/50 border-border/50 flex-1" 
                        value={platformsForm[platform.field]} 
                        onChange={(e) => updatePlatformField(platform.field, e.target.value)} 
                        placeholder="https://..." 
                      />
                      <Button 
                        variant="outline" 
                        ...
                        disabled={!platformsForm[platform.field]} 
                        onClick={() => window.open(platformsForm[platform.field], "_blank", "noopener,noreferrer")}
                      >
              ...
              <Button 
                size="sm" 
                disabled={isLoadingGeneral || isSavingPlatforms} 
                className="mt-6 glow-red-hover" 
                onClick={savePlatformsSettings}
              >
                {isSavingPlatforms && <Loader2 size={14} className="mr-2 animate-spin" />}
            ...
            <motion.div variants={fadeUp} custom={0} className="rounded-xl border border-border/50 bg-gradient-card p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid gap-2">
                    <div className="relative">
                      <Input 
                        ...
                        placeholder={t("sett.current_password") || "Current password"} 
                    ...
                    <Input 
                      className="bg-secondary/50 border-border/50" 
                      type={showPassword ? "text" : "password"} 
                      placeholder={t("sett.new_password")} 
                    ...
                    <Input 
                      className="bg-secondary/50 border-border/50" 
                      type={showPassword ? "text" : "password"} 
                      placeholder={t("sett.confirm_password") || "Confirm new password"} 
                  ...
                  >
                    {isChangingPassword && <Loader2 size={14} className="mr-2 animate-spin" />}
                    {t("sett.update")}
                  </Button>
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg bg-secondary/30 p-4">
                  ...
                  <Toggle 
                    value={securityForm.twoFactorEnabled} 
                    onChange={(value) => setSecurityToggle("twoFactorEnabled", value)} 
                  />
                </div>
                    ...
                    <div className="rounded-lg bg-secondary/20 p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-medium block">{t("sett.setup_email")}</span>
                            <span className="text-[11px] text-muted-foreground">{t("sett.email_2fa_desc")}</span>
                          </div>
                        </div>
                        <Button 
                          size="sm" 
                          variant={securityForm.twoFactorEmail ? "default" : "outline"} 
                          className={securityForm.twoFactorEmail ? "glow-red-hover" : "border-border/50"} 
                          onClick={() => setSecurityToggle("twoFactorEmail", !securityForm.twoFactorEmail)}
                        >
                          {securityForm.twoFactorEmail ? t("sett.enabled") : t("sett.enable")}
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="rounded-lg bg-secondary/20 p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-medium block">{t("sett.setup_phone")}</span>
                            <span className="text-[11px] text-muted-foreground">{t("sett.phone_2fa_desc")}</span>
                          </div>
                        </div>
                        <Button 
                          size="sm" 
                          variant={securityForm.twoFactorPhone ? "default" : "outline"} 
                          className={securityForm.twoFactorPhone ? "glow-red-hover" : "border-border/50"} 
                          onClick={() => setSecurityToggle("twoFactorPhone", !securityForm.twoFactorPhone)}
                        >
                          {securityForm.twoFactorPhone ? t("sett.enabled") : t("sett.enable")}
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="rounded-lg bg-secondary/20 p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-medium block">{t("sett.setup_google")}</span>
                            <span className="text-[11px] text-muted-foreground">{t("sett.google_2fa_desc")}</span>
                          </div>
                        </div>
                        <Button 
                          size="sm" 
                          variant={securityForm.twoFactorGoogle ? "default" : "outline"} 
                          className={securityForm.twoFactorGoogle ? "glow-red-hover" : "border-border/50"} 
                          onClick={() => setSecurityToggle("twoFactorGoogle", !securityForm.twoFactorGoogle)}
                        >
                          {securityForm.twoFactorGoogle ? t("sett.enabled") : t("sett.enable")}
                        </Button>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input 
                      className="bg-secondary/50 border-border/50 flex-1 font-mono text-xs" 
                      value="rv_live_sk_xxxxxxxxxxxxxxxxxxxxxxxx" 
                      readOnly 
                    />
                    <Button 
                      variant="outline" 
                      size="icon" 
                      className="border-border/50" 
                      onClick={() => handleCopy("rv_live_sk_xxxxxxxxxxxxxxxxxxxxxxxx")}
                    >
                      <Copy size={14} />
                    </Button>
                  </div>
                ...
                <Button 
                  size="sm" 
                  disabled={isLoadingGeneral || isSavingSecurity} 
                  className="glow-red-hover" 
                  onClick={saveSecuritySettings}
                >
                  {isSavingSecurity && <Loader2 size={14} className="mr-2 animate-spin" />}
                  {t("sett.save_changes")}
                </Button>
              </div>
            </motion.div>
          ...
          <TabsContent value="subscription">
            <motion.div variants={fadeUp} custom={0} className="rounded-xl border border-border/50 bg-gradient-card p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="font-display font-semibold">{t("sett.subscription_title")}</h3>
                  <p className="text-xs text-muted-foreground">{t("sett.subscription_desc")}</p>
                </div>
              </div>
              ...
              {loadingSubscription ? (
                ...
                <>
                  <div className="rounded-lg bg-primary/5 border border-primary/20 p-4 mb-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-primary/20 text-primary capitalize">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      ...
                      {subscription?.nextBilling 
                        ? ` • Next billing: ${new Date(subscription.nextBilling).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}` 
                        : ""}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    >
                      <Plus size={14} /> {t("sett.add_locations")}
                    </Button>
                    <Button 
                      variant="ghost" 
                      size="sm" 
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {upgradeStep === "select" ? t("sett.upgrade_plan") : "Confirm Plan Change"}
            </DialogTitle>
          </DialogHeader>
            ...
            <>
              <div className="flex items-center justify-center gap-3 rounded-lg bg-secondary/30 p-3 mb-1">
                ...
                <button 
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${annualBilling ? "bg-primary" : "bg-muted"}`}
                >
              ...
              <div className="space-y-3">
                {loadingPlans ? (
                  ...
                  availablePlans.map((plan) => {
                    const displayPrice = annualBilling ? plan.annual : plan.price;
                    ...
                    return (
                      <div 
                        key={plan.id} 
                        onClick={() => !isCurrent && setSelectedPlan({ ...plan, price: priceLabel, rawPrice: displayPrice })} 
                        className={`rounded-lg border p-4 cursor-pointer transition-all ${
                          isCurrent 
                            ? "border-primary bg-primary/5 opacity-60 cursor-default" 
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {isCurrent && (
                              <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-primary/20 text-primary">
                                {t("sett.current")}
                              </span>
                            )}
                            {selectedPlan?.id === plan.id && !isCurrent && (
                              <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-primary/20 text-primary">
                                Selected
                              </span>
                            )}
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {plan.locationLimit} Locations, {plan.apiLimit} API calls/mo
                        </p>
                        {annualBilling && yearlyTotal && (
                          <p className="text-[11px] text-primary mt-1 font-medium">
                            Billed ${yearlyTotal.toLocaleString()}/year — Save ${(plan.price - displayPrice) * 12}/yr
                          </p>
                        )}
                      </div>
                    );
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{subscription?.planName || "Free"} — ${subscription?.amount || 0}/mo</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">{annualBilling ? "Annual" : "Monthly"}</span>
                  </div>
                  {annualBilling && (
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Annual total</span>
                      <span className="font-semibold text-primary">
                        ${(selectedPlan.rawPrice * 12).toLocaleString()}/yr
                      </span>
                    </div>
                  )}
                  <div className="border-t border-border/30 pt-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {selectedPlan.locationLimit} Locations, {selectedPlan.apiLimit} API calls/mo
                    </p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  Your plan will be updated immediately. Billing will be prorated for the current period.
                </p>
              </div>

              <DialogFooter>
                <Button variant="outline" onClick={() => setUpgradeStep("select")}>
                  Back
                </Button>
                <Button 
                  className="glow-red-hover" 
      ...
      <Dialog 
        open={showLocations} 
        onOpenChange={(open) => {
          setShowLocations(open);
          if (!open) setLocationStep("input");
        }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {locationStep === "input" ? t("sett.add_locations") : "Confirm Your Selection"}
            </DialogTitle>
          </DialogHeader>
            ...
            <>
              <div className="space-y-4">
                <div className="rounded-lg bg-secondary/30 p-4">
                  <p className="text-sm">
                    Current: <strong>{subscription?.locationCount || 0} locations</strong>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Each additional location costs $29/mo
                  </p>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground mb-1.5 block">
                  ...
                  <Input 
                    ...
                    min={1} 
                    onChange={(e) => setLocationsToAdd(Math.max(1, parseInt(e.target.value) || 1))} 
                    className="bg-secondary/50 border-border/50" 
                  />
                </div>
              </div>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setShowLocations(false)}>
                  {t("sett.cancel")}
                </Button>
                <Button className="glow-red-hover" onClick={() => setLocationStep("confirm")}>
                  Continue
                </Button>
              </DialogFooter>
            </>
            ...
            <>
              <div className="space-y-4">
                <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Locations to add</span>
                    <span className="font-semibold">{locationsToAdd}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Cost per location</span>
                    <span>$29/mo</span>
                  </div>
                  <div className="border-t border-border/30 pt-2 flex justify-between text-sm font-semibold">
                    <span>Additional monthly cost</span>
                    <span className="text-primary">${locationsToAdd * 29}/mo</span>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  You'll be redirected to the Add-Ons page to finalize your subscription changes.
                </p>
              </div>

              <DialogFooter>
                <Button variant="outline" onClick={() => setLocationStep("input")}>
                  Back
                </Button>
                <Button 
                  className="glow-red-hover" 
      ...
      <Dialog open={showBillingHistory} onOpenChange={setShowBillingHistory}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t("sett.billing_history")}</DialogTitle>
          </DialogHeader>
              ...
              invoices.map((inv) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {inv.items?.[0]?.service || inv.invoiceNumber}
                    </p>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-medium">
