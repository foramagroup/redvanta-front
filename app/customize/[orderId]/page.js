    ...
    <div>
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <Input
          ...
          className="h-8 text-xs font-mono bg-background border-border/50 w-24"
          maxLength={7}
          placeholder="#000000"
        />
      </div>
...
const CharInput = ({ value, onChange, max, placeholder }) => (
  <div>
    <Input
      value={value}
...
// ── Stars row ───────────────────────────────────────────────
const StarsRow = ({ color = "#FBBF24", size = 12 }) => (
...
const CardPreview = ({ design, orientation, side, frontLine1, frontLine2, backLine1, backLine2, gradient1, gradient2, accentBand1, accentBand2, pattern, bandPosition, colorMode, nameFont, sloganFont, nameFontSize, sloganFontSize, nameLetterSpacing, sloganLetterSpacing, nameTextTransform, sloganTextTransform, nameLineHeight, sloganLineHeight, nameTextAlign, sloganTextAlign, qrPosition, logoPosition, logoSize, qrSize, instructionFont, instructionFontSize, instructionLetterSpacing, instructionLineHeight, instructionTextAlign, nameFontWeight, sloganFontWeight, instructionFontWeight, checkStrokeWidth, starsColor, iconsColor, nfcIconSize, showNfcIcon, showGoogleIcon, frontBandHeight, backBandHeight, textShadow, ctaPaddingTop, googleIconSize, dragMode, elementOffsets, onElementDrag }) => {
  ...
  const renderDraggable = (id, children, className = "") => {
    ...
    return (
      <motion.div
        ...
        data-drag-id={id}
        className={className}
        whileDrag={{ boxShadow: "0 0 12px rgba(225,6,0,0.4)", outline: "2px dashed rgba(255,255,255,0.35)", outlineOffset: 0 }}
      >
        {children}
        {isThisDragging && (
          <div
            className="absolute -top-6 left-1/2 -translate-x-1/2 pointer-events-none whitespace-nowrap rounded bg-card/90 border border-border/50 px-1.5 py-0.5 text-[9px] font-mono text-primary shadow-sm backdrop-blur-sm"
            style={{ zIndex: 50 }}
          >
  ...
  const AlignmentGuides = () => {
    if (!dragMode || !isDragging) return null;
    return (
      <div className="absolute inset-0 z-[50] pointer-events-none overflow-hidden">
        ...
        {activeGuides.x !== null && (
          <motion.div
            ...
            style={{ left: `${activeGuides.x}%`, background: 'hsl(var(--primary))', boxShadow: '0 0 6px hsl(var(--primary) / 0.5)' }}
          />
        )}
        {activeGuides.y !== null && (
          <motion.div
            ...
            style={{ top: `${activeGuides.y}%`, background: 'hsl(var(--primary))', boxShadow: '0 0 6px hsl(var(--primary) / 0.5)' }}
          />
        )}
      </div>
    );
  ...
  const logoImage = design.logoUrl ? (
    <img
      src={design.logoUrl}
      alt="Logo"
      draggable={false}
  ...
  if (side === "front") {
    if (isLandscape) {
      ...
      const logoElement = design.logoUrl ? (
        <div className="shrink-0 flex items-center">{logoImage}</div>
      ) : null;

      const businessInfoBlock = (
        <div className="flex flex-col justify-center" style={{ textAlign: nameTextAlign }}>
      ...
      const frontCtaBlock = (
        <div style={{ paddingTop: `${ctaPaddingTop}px` }}>
      ...
      return (
        <div ref={cardRef} className={`${aspectClass} rounded-xl overflow-hidden shadow-2xl relative transition-all`} style={bgStyle}>
          <AlignmentGuides />
          {colorMode === "template" && <PatternOverlay pattern={pattern} />}
          {showNfcIcon && renderDraggable("nfcIcon",
            <svg width={nfcIconSize} height={nfcIconSize} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            "absolute top-3 right-3 opacity-30 z-20"
          )}
          {showGoogleIcon && renderDraggable("googleIcon",
            <GoogleIcon size={googleIconSize} />,
            "absolute bottom-3 right-3 opacity-60 z-20"
          )}
          ...
          {colorMode === "template" && bandPosition !== "hidden" && (
            <div
              className={`absolute left-0 right-0 ${bandPosition === "top" ? "top-0" : "bottom-0"}`}
              style={{ height: `${frontBandHeight}%`, background: `linear-gradient(90deg, ${accentBand1} 0%, ${accentBand2} 100%)`, opacity: 0.9 }}
            />
          )}
        </div>
      );
    ...
    return (
      <div ref={cardRef} className={`${aspectClass} rounded-xl overflow-hidden shadow-2xl relative transition-all`} style={bgStyle}>
        ...
        <div className="h-full flex flex-col justify-between relative z-10 p-5">
          {design.logoUrl && logoPosition === "top-center" && renderDraggable("logo",
            <div className="flex justify-center">
              {logoImage}
            </div>,
            "z-20"
          )}
          {renderDraggable("businessInfo",
            <div style={{ textAlign: nameTextAlign }}>
              <p style={{ fontSize: `${nameFontSize}px`, fontFamily: nameFont, fontWeight: nameFontWeight, letterSpacing: nameSpacing, textTransform: nameTextTransform === "none" ? undefined : nameTextTransform, lineHeight: nameLineHeight, textAlign: nameTextAlign, textShadow: textShadowStyle }}>{design.businessName || "Business Name"}</p>
              <StarsRow color={starsColor} size={14} />
          ...
          {renderDraggable("instructions",
            <div className="space-y-1.5 my-2">
              {frontLine1 && (
                <div className={`flex items-center gap-1.5 ${instructionTextAlign === "center" ? "justify-center" : instructionTextAlign === "right" ? "justify-end" : "justify-start"}`}>
                  <Check size={12} strokeWidth={checkStrokeWidth} className="shrink-0" style={{ color: iconsColor }} />
                  <span className="opacity-90" style={{ fontSize: `${instructionFontSize}px`, fontFamily: instructionFont, fontWeight: instructionFontWeight, letterSpacing: instrSpacing, lineHeight: instructionLineHeight, textShadow: textShadowStyle }}>{frontLine1}</span>
                </div>
              )}
          ...
          {design.logoUrl && logoPosition === "bottom-center" && renderDraggable("logo",
          ...
          {renderDraggable("cta",
            <div className="flex items-end justify-between" style={{ paddingTop: `${ctaPaddingTop}px` }}>
              <p className="text-xs font-medium opacity-80" style={{ textShadow: textShadowStyle }}>{design.cta || "Powered by RedVanta"}</p>
            </div>
          )}
        </div>
  ...
  // ── Back side ───────────────────────────────────────────
  const qrElement = (
    <div className="rounded-lg flex items-center justify-center shrink-0" style={{ height: `${qrSize}px`, width: `${qrSize}px`, backgroundColor: design.qrColor + "18", border: `1px solid ${design.qrColor}33` }}>
  ...
  const secondGroup = (
    <div className="flex flex-col items-center gap-1.5">
      {backLine1 && (
        <div className={`flex items-center gap-1.5 ${backInstrJustify}`}>
  ...
  return (
    <div ref={cardRef} className={`${aspectClass} rounded-xl overflow-hidden shadow-2xl relative transition-all`} style={bgStyleBack}>
      <AlignmentGuides />
      {colorMode === "template" && <PatternOverlay pattern={pattern} />}
      {showGoogleIcon && renderDraggable("googleIcon",
        <GoogleIcon size={googleIconSize} />,
      ...
      {colorMode === "template" && bandPosition !== "hidden" && (
        <div
          className={`absolute left-0 right-0 ${bandPosition === "top" ? "top-0" : "bottom-0"}`}
          style={{ height: `${backBandHeight}%`, background: `linear-gradient(90deg, ${accentBand1} 0%, ${accentBand2} 100%)`, opacity: 0.9 }}
        />
      )}
    </div>
  );
...
const Customize = () => {
  ...
  const [frontLine2, setFrontLine2] = useState(() => editingDesign?.frontInstructions.split("\n")[1] || "Tap to leave a review");
  const [backLine1, setBackLine1] = useState(() => editingDesign?.backInstructions.split("\n")[0] || "Use your phone camera to scan the QR code");
  const [backLine2, setBackLine2] = useState(() => editingDesign?.backInstructions.split("\n")[1] || "Write a review on our Google Maps page");

  // Template / gradient state - pre-load from editing design
  const matchingTemplate = editingDesign ? CARD_TEMPLATES.find(t => t.label === editingDesign.template) : null;
  const [selectedTemplate, setSelectedTemplate] = useState(() => matchingTemplate?.id || "crimson-noir");
  const [templateFilter, setTemplateFilter] = useState("all");
  const activeTemplate = CARD_TEMPLATES.find(t => t.id === selectedTemplate) || CARD_TEMPLATES[0];
  const [gradient1, setGradient1] = useState(() => editingDesign?.templateColor1 || activeTemplate.gradient1);
  const [gradient2, setGradient2] = useState(() => editingDesign?.templateColor2 || activeTemplate.gradient2);
  const [accentBand1, setAccentBand1] = useState(activeTemplate.accentBand1);
  const [accentBand2, setAccentBand2] = useState(activeTemplate.accentBand2);
  ...
  useEffect(() => {
    if (orientation === "landscape") {
      if (qrPosition !== "left" && qrPosition !== "right") setQrPosition("right");
      if (logoPosition !== "left" && logoPosition !== "right") {
        setLogoPosition("left");
      }
    } else {
      if (qrPosition !== "top" && qrPosition !== "bottom") setQrPosition("top");
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-dark pt-20">
        <div className="text-center">
          ...
          <Button onClick={() => router.push("/cart")} className="mt-4">{t("customize.back_to_cart")}</Button>
        </div>
      </div>
    );
  }
  ...
  const applyTheme = (themeId) => {
    ...
    if (t) setDesign((d) => ({ ...d, theme: themeId, bgColor: t.bg, textColor: t.text, qrColor: t.qr }));
  };
  ...
  const saveAndContinue = () => {
    ...
    if (item) {
      ...
      setTimeout(() => router.push("/cross-sell"), 500);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-dark pt-32 pb-20">
        ...
        <motion.div initial="hidden" animate="visible">
          ...
          <motion.p variants={fadeUp} custom={1} className="mt-2 text-muted-foreground">
            {isEditMode && editingDesign
              ...
              : `${t("customize.subtitle_prefix")} ${item?.productName || ""}`}
          </motion.p>
        </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" className="lg:col-span-3 space-y-8 order-2 lg:order-1">
              ...
              <>
                <motion.div variants={fadeUp} custom={2} className="rounded-xl border border-border/50 bg-gradient-card p-6">
                  <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
                    <Search size={18} className="text-primary" /> {t("customize.find_business")}
                  </h3>
                  <p className="mt-1 text-sm text-muted-foreground">{t("customize.find_desc")}</p>
            ...
            {currentStep === 1 && (
              <>
                ...
                <motion.div variants={fadeUp} custom={2} className="rounded-xl border border-border/50 bg-gradient-card p-6">
                  <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
                    <RotateCcw size={18} className="text-primary" /> {t("customize.orientation")}
                  </h3>
                  <div className="mt-4 flex gap-3">
                    {["landscape", "portrait"].map(o => (
                      <button key={o} onClick={() => setOrientation(o)}
                        className={`flex-1 rounded-lg border p-4 text-center transition-all ${orientation === o ? "border-primary/50 bg-primary/10" : "border-border/50 hover:border-border"}`}>
                        <div className={`mx-auto rounded border border-muted-foreground/30 ${o === "landscape" ? "w-16 h-10" : "w-10 h-16"}`} />
                        <span className="mt-2 block text-sm font-medium capitalize">{o === "landscape" ? t("customize.landscape") : t("customize.portrait")}</span>
                      </button>
                    ))}
                  </div>
                </motion.div>
                          ...
                          {LOGO_POSITION_OPTIONS
                            ...
                            <button key={pos.id} onClick={() => setLogoPosition(pos.id)}
                              className={`rounded-lg border p-2 text-center text-xs font-medium transition-all ${
                                logoPosition === pos.id ? "border-primary/50 bg-primary/10" : "border-border/50 hover:border-border"
                              }`}>
                              {pos.label}
                            </button>
                          ))}
                          ...
                          {LOGO_SIZE_PRESETS.map((s) => (
                            <button key={s} onClick={() => setLogoSize(s)}
                              className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${
                                logoSize === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"
                              }`}>{s}</button>
                          ))}
                          <Input type="number" min={16} max={80} value={logoSize}
                            onChange={(e) => { const v = parseInt(e.target.value); if (v >= 16 && v <= 80) setLogoSize(v); }}
                ...
                <motion.div variants={fadeUp} custom={4} className="rounded-xl border border-border/50 bg-gradient-card p-6 space-y-4">
                  ...
                  <div className="flex gap-3">
                    {([
                      ...
                      <button key={mode.id} onClick={() => setColorMode(mode.id)}
                        className={`flex-1 rounded-lg border p-4 text-left transition-all ${colorMode === mode.id ? "border-primary/50 bg-primary/10" : "border-border/50 hover:border-border"}`}>
                  ...
                  {colorMode === "single" && (
                    <div className="space-y-4 pt-2">
                      ...
                      <div>
                        ...
                        <div className="mt-2 grid grid-cols-4 gap-2">
                          {SINGLE_COLOR_PRESETS.map((preset) => (
                            <button
                              key={preset.label}
                              onClick={() => setDesign(d => ({ ...d, bgColor: preset.bg, textColor: preset.text, qrColor: preset.qr }))}
                              className={`rounded-lg border p-2 text-center transition-all ${
                                design.bgColor === preset.bg && design.textColor === preset.text && design.qrColor === preset.qr
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex gap-1 flex-wrap items-center">
                          {NFC_SIZE_PRESETS.map((s) => (
                            <button key={s} onClick={() => setNfcIconSize(s)}
                              className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${nfcIconSize === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>{s}</button>
                          ))}
                          <Input type="number" min={12} max={48} value={nfcIconSize}
                            onChange={(e) => { const v = parseInt(e.target.value); if (v >= 12 && v <= 48) setNfcIconSize(v); }}
                            className="w-16 h-7 text-xs text-center" placeholder="px" />
                        </div>
                      </div>
                      {/* Icon Visibility Toggles */}
                      <div className="flex gap-4">
                        <button onClick={() => setShowNfcIcon(!showNfcIcon)}
                          className={`flex-1 rounded-lg border p-3 text-center text-xs font-medium transition-all ${showNfcIcon ? "border-primary/50 bg-primary/10" : "border-border/50 opacity-50"}`}>
                          {showNfcIcon ? t("customize.nfc_on") : t("customize.nfc_off")}
                        </button>
                        <button onClick={() => setShowGoogleIcon(!showGoogleIcon)}
                          className={`flex-1 rounded-lg border p-3 text-center text-xs font-medium transition-all ${showGoogleIcon ? "border-primary/50 bg-primary/10" : "border-border/50 opacity-50"}`}>
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        {CARD_TEMPLATES
                          ...
                          <button
                            ...
                            className={`rounded-lg border p-3 transition-all ${selectedTemplate === tpl.id ? "border-primary/50 bg-primary/10 glow-red-sm" : "border-border/50 hover:border-border"}`}
                          >
                            <div className="aspect-[1.6/1] rounded-md overflow-hidden relative mb-2"
                              style={{ background: `linear-gradient(160deg, ${tpl.gradient1} 0%, ${tpl.gradient2} 70%)` }}>
                              <PatternOverlay pattern={tpl.pattern} />
                              {bandPosition !== "hidden" && <div className={`absolute left-0 right-0 h-[22%] ${bandPosition === "top" ? "top-0" : "bottom-0"}`}
                                style={{ background: `linear-gradient(90deg, ${tpl.accentBand1} 0%, ${tpl.accentBand2} 100%)`, opacity: 0.9 }} />}
                              <div className="p-2 relative z-10">
                                <div className="h-1.5 w-10 rounded bg-white/60" />
                                <div className="h-1 w-6 rounded bg-white/30 mt-1" />
                              </div>
                              {tpl.badge && (
                                <span className="absolute top-1 right-1 text-[8px] font-bold px-1.5 py-0.5 rounded bg-primary/90 text-primary-foreground z-10">
                                  {tpl.badge}
                                </span>
                              )}
                            </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex gap-3">
                          {["top", "bottom", "hidden"].map(pos => (
                            <button key={pos} onClick={() => setBandPosition(pos)}
                              className={`flex-1 rounded-lg border p-3 text-center text-sm font-medium capitalize transition-all ${
                                bandPosition === pos ? "border-primary/50 bg-primary/10" : "border-border/50 hover:border-border"
                              }`}>
                      ...
                      {bandPosition !== "hidden" && (
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <div className="mt-1 flex gap-1 flex-wrap items-center">
                              {BAND_HEIGHT_PRESETS.map((s) => (
                                <button key={s} onClick={() => setFrontBandHeight(s)}
                                  className={`rounded-md border px-2 py-1 text-xs font-medium transition-all ${frontBandHeight === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>{s}%</button>
                              ))}
                              <Input type="number" min={4} max={50} value={frontBandHeight}
                                onChange={(e) => { const v = parseInt(e.target.value); if (v >= 4 && v <= 50) setFrontBandHeight(v); }}
                                className="w-16 h-7 text-xs text-center" placeholder="%" />
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="mt-1 flex gap-1 flex-wrap items-center">
                              {BAND_HEIGHT_PRESETS.map((s) => (
                                <button key={s} onClick={() => setBackBandHeight(s)}
                                  className={`rounded-md border px-2 py-1 text-xs font-medium transition-all ${backBandHeight === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>{s}%</button>
                              ))}
                              <Input type="number" min={4} max={50} value={backBandHeight}
                                onChange={(e) => { const v = parseInt(e.target.value); if (v >= 4 && v <= 50) setBackBandHeight(v); }}
                                className="w-16 h-7 text-xs text-center" placeholder="%" />
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                        {FONT_OPTIONS.map((font) => (
                          <button
                            key={font.id}
                            onClick={() => setNameFont(font.family)}
                      ...
                      <div className="mt-2">
                        ...
                        <div className="mt-1 flex gap-1 flex-wrap items-center">
                          {NAME_FONT_SIZE_PRESETS.map((s) => (
                            <button
                              key={s}
                              onClick={() => setNameFontSize(s)}
                              className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${
                                nameFontSize === s
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {FONT_WEIGHT_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setNameFontWeight(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${nameFontWeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {LETTER_SPACING_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setNameLetterSpacing(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${nameLetterSpacing === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {TEXT_TRANSFORM_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setNameTextTransform(o.id)}
                                className={`rounded-md border px-2.5 py-1 text-[10px] font-medium transition-all ${nameTextTransform === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {LINE_HEIGHT_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setNameLineHeight(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${nameLineHeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {TEXT_ALIGN_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setNameTextAlign(o.id)}
                                className={`rounded-md border px-2.5 py-1 text-[10px] font-medium transition-all ${nameTextAlign === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                        {FONT_OPTIONS.map((font) => (
                          <button
                            key={font.id}
                            onClick={() => setSloganFont(font.family)}
                            className={`rounded-lg border p-3 text-left transition-all ${
                              sloganFont === font.family
                                ? "border-primary/50 bg-primary/10 ring-1 ring-primary/30"
                                : "border-border/50 hover:border-border"
                            }`}
                          >
                      ...
                      <div className="mt-2">
                        <label className="text-xs text-muted-foreground font-medium">{t("customize.size_px")}</label>
                        <div className="mt-1 flex gap-1 flex-wrap items-center">
                          {SLOGAN_FONT_SIZE_PRESETS.map((s) => (
                            <button
                              key={s}
                              onClick={() => setSloganFontSize(s)}
                              className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${
                                sloganFontSize === s
                                  ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30"
                                  : "border-border/50 text-muted-foreground hover:border-border"
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const v = parseInt(e.target.value);
                              if (v >= 6 && v <= 36) setSloganFontSize(v);
                            }}
                            className="w-16 h-7 text-xs text-center"
                            placeholder="px"
                          />
                      ...
                      <div className="mt-2 flex gap-4 flex-wrap">
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {FONT_WEIGHT_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setSloganFontWeight(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${sloganFontWeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {LETTER_SPACING_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setSloganLetterSpacing(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${sloganLetterSpacing === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {TEXT_TRANSFORM_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setSloganTextTransform(o.id)}
                                className={`rounded-md border px-2.5 py-1 text-[10px] font-medium transition-all ${sloganTextTransform === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {LINE_HEIGHT_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setSloganLineHeight(o.id)}
                                className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${sloganLineHeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex gap-1">
                            {TEXT_ALIGN_OPTIONS.map((o) => (
                              <button key={o.id} onClick={() => setSloganTextAlign(o.id)}
                                className={`rounded-md border px-2.5 py-1 text-[10px] font-medium transition-all ${sloganTextAlign === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                                {o.label}
                              </button>
                            ))}
                          </div>
                    ...
                    <div className="border-t border-border/30 pt-4">
                      ...
                      <div className="mt-2 flex gap-1 flex-wrap">
                        {TEXT_SHADOW_OPTIONS.map((o) => (
                          <button key={o.id} onClick={() => setTextShadow(o.id)}
                            className={`rounded-md border px-2.5 py-1.5 text-xs font-medium transition-all ${textShadow === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                            {o.label}
                          </button>
                        ))}
                      </div>
                ...
                <motion.div variants={fadeUp} custom={6} className="rounded-xl border border-border/50 bg-gradient-card p-6 space-y-4">
                  ...
                  <div className="border-t border-border/30 pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {FONT_OPTIONS.map((font) => (
                        <button
                          key={font.id}
                          onClick={() => setInstructionFont(font.family)}
                          className={`rounded-lg border p-2 text-left transition-all ${
                            instructionFont === font.family
                              ? "border-primary/50 bg-primary/10 ring-1 ring-primary/30"
                              : "border-border/50 hover:border-border"
                          }`}
                        >
                    ...
                    <div>
                      <label className="text-xs text-muted-foreground font-medium">{t("customize.size_px")}</label>
                      <div className="mt-1 flex gap-1 flex-wrap items-center">
                        {INSTRUCTION_FONT_SIZE_PRESETS.map((s) => (
                          <button
                            key={s}
                            onClick={() => setInstructionFontSize(s)}
                            className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${
                              instructionFontSize === s
                                ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30"
                                : "border-border/50 text-muted-foreground hover:border-border"
                            }`}
                          >
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const v = parseInt(e.target.value);
                            if (v >= 6 && v <= 24) setInstructionFontSize(v);
                          }}
                          className="w-16 h-7 text-xs text-center"
                          placeholder="px"
                        />
                    ...
                    <div className="flex gap-4 flex-wrap">
                      <div>
                        ...
                        <div className="mt-1 flex gap-1">
                          {FONT_WEIGHT_OPTIONS.map((o) => (
                            <button key={o.id} onClick={() => setInstructionFontWeight(o.id)}
                              className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${instructionFontWeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                              {o.label}
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex gap-1">
                          {LETTER_SPACING_OPTIONS.map((o) => (
                            <button key={o.id} onClick={() => setInstructionLetterSpacing(o.id)}
                              className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${instructionLetterSpacing === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                              {o.label}
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex gap-1">
                          {LINE_HEIGHT_OPTIONS.map((o) => (
                            <button key={o.id} onClick={() => setInstructionLineHeight(o.id)}
                              className={`rounded-md border px-2 py-1 text-[10px] font-medium transition-all ${instructionLineHeight === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                              {o.label}
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex gap-1">
                          {TEXT_ALIGN_OPTIONS.map((o) => (
                            <button key={o.id} onClick={() => setInstructionTextAlign(o.id)}
                              className={`rounded-md border px-2.5 py-1 text-[10px] font-medium transition-all ${instructionTextAlign === o.id ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>
                              {o.label}
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          <input type="range" min={1} max={5} step={0.5} value={checkStrokeWidth} onChange={(e) => setCheckStrokeWidth(Number(e.target.value))}
                  ...
                  <div className="border-t border-border/30 pt-4 space-y-4">
                    <div>
                      ...
                      <div className="mt-2 flex gap-2">
                        {ELEMENT_POSITION_OPTIONS
                          ...
                          .map(pos => (
                          <button key={pos.id} onClick={() => setQrPosition(pos.id)}
                            className={`flex-1 rounded-lg border p-2 text-center text-xs font-medium transition-all ${
                              qrPosition === pos.id ? "border-primary/50 bg-primary/10" : "border-border/50 hover:border-border"
                            }`}>
                            {pos.label}
                          </button>
                        ))}
                      ...
                      <div className="mt-1 flex gap-1 flex-wrap items-center">
                        {QR_SIZE_PRESETS.map((s) => (
                          <button key={s} onClick={() => setQrSize(s)}
                            className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${
                              qrSize === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"
                            }`}>{s}</button>
                        ))}
                        <Input type="number" min={32} max={120} value={qrSize}
                          onChange={(e) => { const v = parseInt(e.target.value); if (v >= 32 && v <= 120) setQrSize(v); }}
                          className="w-16 h-7 text-xs text-center" placeholder="px" />
                      </div>
                    </div>
                   </div>
                   ...
                   <div className="border-t border-border/30 pt-4">
                     ...
                     <div className="mt-1 flex gap-1 flex-wrap items-center">
                       {CTA_PADDING_PRESETS.map((s) => (
                         <button key={s} onClick={() => setCtaPaddingTop(s)}
                           className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${ctaPaddingTop === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>{s}</button>
                       ))}
                       <Input type="number" min={0} max={40} value={ctaPaddingTop}
                         onChange={(e) => { const v = parseInt(e.target.value); if (v >= 0 && v <= 40) setCtaPaddingTop(v); }}
                         className="w-16 h-7 text-xs text-center" placeholder="px" />
                     </div>
                   </div>
                   ...
                   <div className="border-t border-border/30 pt-4">
                     ...
                     <div className="mt-1 flex gap-1 flex-wrap items-center">
                       {GOOGLE_ICON_SIZE_PRESETS.map((s) => (
                         <button key={s} onClick={() => setGoogleIconSize(s)}
                           className={`rounded-md border px-2.5 py-1 text-xs font-medium transition-all ${googleIconSize === s ? "border-primary/50 bg-primary/10 text-primary ring-1 ring-primary/30" : "border-border/50 text-muted-foreground hover:border-border"}`}>{s}</button>
                       ))}
                       <Input type="number" min={12} max={48} value={googleIconSize}
                         onChange={(e) => { const v = parseInt(e.target.value); if (v >= 12 && v <= 48) setGoogleIconSize(v); }}
                         className="w-16 h-7 text-xs text-center" placeholder="px" />
                     </div>
                   </div>
                 </motion.div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {MODELS.map((m) => (
                      <button key={m} onClick={() => changeModel(m)}
                        className={`rounded-lg border p-4 text-center transition-all ${design.model === m ? "border-primary/50 bg-primary/10 glow-red-sm" : "border-border/50 hover:border-border"}`}>
            ...
            <div className="flex gap-3 flex-wrap">
              {isEditMode && currentStep === 0 && (
                <Button variant="outline" onClick={() => safeNavigate("/dashboard/designs")}>
              ...
              {currentStep === 2 && (
                <Button onClick={saveAndContinue} className="glow-red-hover bg-primary text-primary-foreground hover:bg-primary/90">
              ...
              <Button variant="ghost" className="gap-2 text-muted-foreground hover:text-foreground" onClick={() => {
                ...
                setTimeout(() => router.push("/dashboard/designs"), 500);
              }}>
                <Save size={16} /> {t("customize.save_draft") || "Save as Draft"}
              </Button>
              {/* Save / Update (from step 1+) */}
              {currentStep >= 1 && (
                <Button variant="outline" className="gap-2 border-border/50" onClick={() => {
                  const now = new Date().toISOString().split("T")[0];
                  const designData = {
                    name: design.businessName ? `${design.businessName} Design` : "Untitled Design",
                    businessName: design.businessName || "Untitled",
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {dragMode && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-secondary text-muted-foreground hover:text-foreground border border-border/50 transition-all"
                      >
                    ...
                    <div className="flex rounded-lg border border-border/50 overflow-hidden">
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-medium transition-colors ${previewSide === "front" ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
                      >
                        {t("customize.front")}
                      </button>
                      <button
                        onClick={() => setPreviewSide("back")}
                        className={`px-3 py-1.5 text-xs font-medium transition-colors ${previewSide === "back" ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
                      >
                        {t("customize.back")}
                      </button>
                    </div>
                ...
                <CardPreview
                  ...
                  gradient1={gradient1}
                  gradient2={gradient2}
                  accentBand1={accentBand1}
                  accentBand2={accentBand2}
                  pattern={activeTemplate.pattern}
                  ...
                  sloganTextTransform={sloganTextTransform}
                  nameLineHeight={nameLineHeight}
                  sloganLineHeight={sloganLineHeight}
                  ...
                  instructionLetterSpacing={instructionLetterSpacing}
                  instructionLineHeight={instructionLineHeight}
                  instructionTextAlign={instructionTextAlign}
                  nameFontWeight={nameFontWeight}
                  sloganFontWeight={sloganFontWeight}
                  instructionFontWeight={instructionFontWeight}
                  checkStrokeWidth={checkStrokeWidth}
                  starsColor={starsColor}
                  iconsColor={iconsColor}
                  nfcIconSize={nfcIconSize}
                  showNfcIcon={showNfcIcon}
                  showGoogleIcon={showGoogleIcon}
                  frontBandHeight={frontBandHeight}
                  backBandHeight={backBandHeight}
                  textShadow={textShadow}
                  ctaPaddingTop={ctaPaddingTop}
                  googleIconSize={googleIconSize}
                  dragMode={dragMode}
      ...
      <Dialog open={showHistoryDialog} onOpenChange={(open) => { setShowHistoryDialog(open); if (!open) setCompareVersion(null); }}>
        <DialogContent className={cn("transition-all", compareVersion ? "sm:max-w-4xl" : "sm:max-w-lg")}>
          ...
          {compareVersion ? (() => {
            ...
            const DiffBadge = () => diffCount > 0 ? (
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-primary/40 text-primary ml-2">{diffCount} {diffCount === 1 ? "change" : "changes"}</Badge>
            ) : (
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-muted-foreground/30 text-muted-foreground ml-2">Identical</Badge>
            );
            const DiffDot = ({ changed }) => changed ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-border p-3 space-y-2">
                    <div
                      className={cn("w-full h-20 rounded-md", diffs.colors && "ring-2 ring-primary/40")}
                      style={{ background: `linear-gradient(135deg, ${gradient1}, ${gradient2})` }}
                    />
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 space-y-2">
                    <div
                      className={cn("w-full h-20 rounded-md", diffs.colors && "ring-2 ring-primary/40")}
                      style={{ background: `linear-gradient(135deg, ${snap.templateColor1}, ${snap.templateColor2})` }}
                    />
                    <div className="space-y-1 text-xs">
              ...
              <div className="space-y-2">
                {versionHistory.map((v, i) => {
                  ...
                  return (
                    <div
                      key={v.timestamp}
                      className="flex items-center gap-3 rounded-lg border border-border/50 p-3 hover:border-primary/30 transition-all group cursor-pointer"
                      onClick={() => setCompareVersion(v)}
                    >
                      <div
                        className="w-10 h-7 rounded shrink-0"
                        style={{ background: `linear-gradient(135deg, ${v.snapshot.templateColor1}, ${v.snapshot.templateColor2})` }}
                      />
                      ...
                      <Button
                        ...
                        onClick={(e) => { e.stopPropagation(); setCompareVersion(v); }}
                      >
