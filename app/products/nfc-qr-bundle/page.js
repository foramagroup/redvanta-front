...
import { Palette, Minus, Plus, Settings, Lock, Truck, CreditCard, Package, Sparkles, CheckCircle2 } from "lucide-react";
...
const PRODUCT_SLUG = "nfc-qr-bundle";
const FALLBACK_PRODUCT = {
  slug: PRODUCT_SLUG,
  title: "NFC + QR Bundle",
  description: "The ultimate review collection bundle - smart card + table stand + 10 QR stickers at a discounted price.",
  price: 69,
...
const ORIGINAL = 87;
const includes = [
  { icon: CreditCard, title: "1x Smart Review Card", desc: "NFC + QR enabled premium card" },
  { icon: Package, title: "1x Premium Table Stand", desc: "Acrylic display stand for your card" },
  { icon: Sparkles, title: "10x QR Stickers", desc: "Waterproof vinyl stickers" },
];
const benefits = [
  { icon: CheckCircle2, title: "Complete Coverage", desc: "Cover every customer touchpoint - counter, window, packaging, and more." },
  { icon: Palette, title: "20% Savings", desc: "Save compared to buying each item separately." },
...
const faqs = [
  ...
  { q: "How much do I save?", a: "You save compared to purchasing each item separately." },
    ...
    <div className="bg-gradient-dark pt-32">
      <motion.div initial={{ y: -100 }} animate={{ y: isSticky ? 0 : -100 }} transition={{ duration: 0.3 }} className="fixed left-0 right-0 top-[110px] z-40 border-b border-border/50 bg-background/95 backdrop-blur-xl">
        <div className="container mx-auto flex items-center justify-between px-6 py-3">
          ...
          <Button className="glow-red-hover bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => setModalOpen(true)} disabled={!canAddToCart}><Settings size={16} className="mr-2" />{t("shop.setup_card")}</Button>
        </div>
      </motion.div>
            ...
            <motion.div initial="hidden" animate="visible">
              ...
              <motion.div variants={fadeUp} custom={3} className="mt-8 space-y-3">
                ...
                {includes.map((item, index) => (
                  <div key={index} className="flex items-center gap-3 rounded-lg border border-border/30 bg-secondary/50 p-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded bg-primary/10"><item.icon size={16} className="text-primary" /></div>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {packageTiers.map((tier, index) => (
                        ...
                        >
                          ...
                          <span className="block text-xs text-muted-foreground">{formatPrice(tier.price)} / unit</span>
                        </button>
                      ))}
                    </div>
                    {isSingleUnitPackage ? (
              ...
              <motion.div variants={fadeUp} custom={5} className="mt-8 flex flex-col items-start gap-3">
                <Button size="lg" className="glow-red-hover bg-primary px-8 text-base text-primary-foreground hover:bg-primary/90" onClick={() => setModalOpen(true)} disabled={!canAddToCart}><Settings size={18} className="mr-2" />{t("shop.setup_card")} — {formatPrice(totalPrice)}</Button>
      ...
      <section className="section-spacing border-t border-border/30"><div className="container mx-auto px-6">
        <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center font-display text-3xl font-bold md:text-4xl">Why the <span className="text-gradient-red">Bundle</span></motion.h2>
