...
import { Copy, Minus, Plus, ShoppingCart, Lock, Truck, Palette, Link2, Shield, ArrowRight } from "lucide-react";
...
const DuplicateCard = () => {
  ...
  const handleAddToCart = () => {
    addItem({ productName: `Duplicate Card (${colors[selectedColor]})`, model: "classic", quantity, unitPrice: PRICE, design: null });
    router.push("/cart");
  };
  ...
  return (
    <div className="bg-gradient-dark pt-32">
      <motion.div initial={{ y: -100 }} animate={{ y: isSticky ? 0 : -100 }} transition={{ duration: 0.3 }} className="fixed top-[110px] left-0 right-0 z-40 border-b border-border/50 bg-background/95 backdrop-blur-xl">
        <div className="container mx-auto flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center"><Copy size={20} className="text-primary" /></div>
            <div><p className="text-sm font-semibold">Duplicate Card</p><p className="text-xs text-muted-foreground">{quantity} × {formatPrice(PRICE)}</p></div>
          </div>
        ...
        <div className="container mx-auto px-6">
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <motion.div initial="hidden" animate="visible">
              <motion.h1 variants={fadeUp} custom={0} className="font-display text-4xl font-bold leading-tight md:text-5xl lg:text-6xl">Duplicate Card <span className="text-gradient-red">(Different Color)</span></motion.h1>
              <motion.p variants={fadeUp} custom={1} className="mt-6 text-lg text-muted-foreground leading-relaxed max-w-xl">Order an additional Smart Review Card in a different color, linked to the same location profile.</motion.p>
              <motion.div variants={fadeUp} custom={2} className="mt-6">
                <p className="text-sm font-medium text-muted-foreground mb-3">{t("shop.select_color")}</p>
                <div className="flex gap-3">
                  {colors.map((c, i) => (
                    <button key={c} onClick={() => setSelectedColor(i)} className={`px-4 py-2 rounded-lg border text-sm transition-all ${selectedColor === i ? "border-primary/50 bg-primary/10 text-primary" : "border-border/50 bg-secondary text-muted-foreground hover:border-border"}`}>{c}</button>
                  ))}
                </div>
              </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="relative">
              ...
              <div className="relative z-10"><ProductGallery images={GALLERY_IMAGES} productName="Duplicate Card" /></div>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-spacing border-t border-border/30"><div className="container mx-auto px-6">
        <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="font-display text-3xl font-bold text-center md:text-4xl">Why Get a <span className="text-gradient-red">Duplicate</span></motion.h2>
